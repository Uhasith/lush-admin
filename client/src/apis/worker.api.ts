...
export const fetchWorkerListApi = async (payload = null) => {
  try {
    let query = '';
    if (payload) {
      const { worker } = payload;
      query = `/users?role=Worker&firstName=${worker}&limit=100&sortBy=createdAt:desc`;
    } else {
      query = `/users?role=Worker&limit=100&sortBy=createdAt:desc`;
    }
