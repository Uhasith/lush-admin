...
export function* fetchWorkerList({
  payload
}: {
  type: typeof FETCH_WORKER_LIST;
  payload: any;
}): any {
  try {
    yield put({ type: START_LOADING });

    const response = yield call(fetchWorkerListApi, payload);
...
export function* createWorker({
  ...
  try {
    ...
    const message = error?.response?.data?.message || 'Worker adding failed';
    yield put({ type: SET_ERROR_MESSAGE, payload: message });
    yield put({ type: END_LOADING });
  }
}
